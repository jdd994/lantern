// SyncSheet — turning on cross-device sync, and the one thing that has to be
// said plainly while doing it: the account and the passphrase are two different
// secrets doing two different jobs.
//
...
// it, so it says so.
//
// One sheet, three apps. Ballast, Hearth, and Grove each grew a copy of this
// screen and the copies drifted only in words, never in structure — so the
// structure lives here once, and each app passes the words that are truly its
// own (what syncs, how strict the guardian defaults are, who a guardian
// naturally is). The default copy in this file is the plainest telling; an app
// that says nothing gets that.

import { useEffect, useState, type FormEvent, type ReactNode } from "react";
import type { RecoveryCircleInfo, RecoveryStatus, PendingForMe } from "@lantern/core/recovery-api";
...
// The words an app is allowed to make its own. Everything omitted falls back
// to the shared plain telling; `noun` is the one thing every app must say —
// the name of the thing that syncs ("vault", "log", "tree").
export type SyncCopy = {
  noun: string;
  // Tail of the connected line, after "Connected as {account}."
  syncsLine?: string;
  // The whole privacy-note element (an app may add its badge or its own vow).
...
// stricter (Ballast: money, not journal entries).
...
// or any one of them alone, ever holding the key.
function GuardiansSection({
  ...
  copy: SyncCopy;
  defaults: Required<GuardianDefaults>;
  ...
  const [rows, setRows] = useState<{ email: string; codeword: string }[]>(
    Array.from({ length: defaults.rows }, () => ({ email: "", codeword: "" })),
  );
        ...
        <>
          {guardianCircle ? (
            ...
            <p>
              ...
              {copy.guardiansPitchExtra ? <> {copy.guardiansPitchExtra}</> : null}
            </p>
          )}
...
// Approving someone else's recovery request. Only shows up when there's
// something to do.
function GuardianApprovalsSection({
  ...
  return (
    <div className="danger-zone">
      <h4 className="pass-head">{copy.helpHeading ?? "Help a friend"}</h4>
      {done.map((d) => (
...
export function SyncSheet({
  ...
  // A vault exists on this device (so it can be connected to a new account). Also
  // gates the change-passphrase form — that needs an unlocked vault.
  canCreate: boolean;
  ...
  // The app's paper-recovery-kit section, if it has one. It needs the vault key
  // to mint (same gate as the passphrase form), but the words and the minting
  // are the app's own — the sheet only knows where it belongs.
  recoveryKit?: ReactNode;
  onClose: () => void;
  guardianCircle: RecoveryCircleInfo | null;
  onSetupGuardians: (guardians: { email: string; codeword: string }[], k: number, delayMs: number) => Promise<string | null>;
  recoveryStatus: RecoveryStatus;
  ...
  const defaults: Required<GuardianDefaults> = {
    rows: guardianDefaults?.rows ?? 3,
    k: guardianDefaults?.k ?? 2,
    delayMs: guardianDefaults?.delayMs ?? DELAY_OPTIONS[1].ms,
  };
        ...
        {account ? (
          ...
          <>
            <p>
              ...
              {copy.syncsLine ??
                `Your ${copy.noun} syncs quietly in the background — nothing is lost if you're offline, it catches up next time.`}
            </p>
            {copy.privacyNote ?? (
              <p className="hint">
            ...
            {/* Deleting the account removes the server copy only — the {noun} on
                this device is untouched. Two taps, so it can't happen by accident. */}
            <div className="danger-zone">
              {confirmDelete ? (
                <>
                  <p className="hint">
          ...
          <>
            <p>
              Keep the same {copy.noun} on your phone and your laptop. This is separate from your
            ...
            <form onSubmit={submit}>
              ...
              <label className="field">
                ...
                <span className="hint">
                  {copy.passwordHint ??
                    "Not your passphrase — a separate secret, just for signing in. Your passphrase never leaves this device."}
                </span>
              </label>
