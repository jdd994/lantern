...
// success means the passphrase was correct (AES-GCM fails loudly on a wrong key,
// so no false positives). `text` is the per-app token — it MUST stay stable for
// an app forever, or existing vaults stop verifying. Each app binds its own in
// its thin crypto adapter (e.g. "ballast-ok").
export async function makeVerifier(key: CryptoKey, text: string): Promise<CipherBlob> {
  return encryptString(key, text);
}

export async function checkVerifier(key: CryptoKey, blob: CipherBlob, text: string): Promise<boolean> {
  try {
    return (await decryptString(key, blob)) === text;
  } catch {
    return false;
  }
}
