...
//
// So a breach of the sync server yields nothing but noise — not an item, not a
// title, not who's bringing what. This screen is where the user opts in, so it
// says so.
        ...
        <>
          {guardianCircle ? (
            ...
            <p>
              ...
              passphrase — without us, or any one of them, ever holding the key. For calendars you keep
              together, the natural guardians are the people you travel with.
            </p>
          )}
...
function GuardianApprovalsSection({
  ...
  // the server stop returning it, so the list refreshes this row away the
  // instant approval succeeds. Without its own memory of what it just did, the
  // "Approved for X" confirmation would never be visible.
  ...
  return (
    <div className="danger-zone">
      <h4 className="pass-head">Help someone back in</h4>
      {done.map((d) => (
    ...
    <div className="sheet-backdrop" onClick={onClose}>
      <div className="sheet" onClick={(e) => e.stopPropagation()}>
        ...
        {account ? (
          <>
            <p>
              Connected as <strong>{account}</strong>. Your plans sync quietly in the
            ...
            {/* Deleting the account removes the server copy only — the plans on
          ...
          <>
            <p>
              Keep the same almanac on your phone and your laptop. This is separate from your
            ...
            <form onSubmit={submit}>
              ...
              {mode === "signin" && !canCreate ? (
                <p className="hint">
                  Signing in downloads your plans to this device. You'll open them with the same
