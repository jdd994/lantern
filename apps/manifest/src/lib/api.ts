// api.ts — Manifest's binding to the shared sync-server client (@lantern/core/api).
// Only the base URL is Manifest-specific; the endpoints are the shared ones. It
// moves opaque ciphertext + non-secret metadata only — never the passphrase,
// never an item, never a title, never who's bringing what.
...
// manifest-server isn't deployed yet; the env override keeps local dev honest
