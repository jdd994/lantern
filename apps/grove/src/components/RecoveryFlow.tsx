...
export function RecoveryFlow({
  ...
  function fmtCountdown(ms: number): string {
    if (ms <= 0) return t`any moment now`;
    const h = Math.floor(ms / 3_600_000);
    const m = Math.ceil((ms % 3_600_000) / 60_000);
    return h > 0 ? t`about ${h}h ${m}m` : t`about ${m}m`;
  }

  async function signIn(e: React.FormEvent) {
    e.preventDefault();
    setError(null);
    if (!email.trim() || !password) return setError(t`Enter your account email and password.`);
    setBusy(true);
    const ok = await onRecoverySignIn(email, password);
    setBusy(false);
    if (!ok) setError(syncError ?? t`Couldn't sign in.`);
  ...
  async function finish(e: React.FormEvent) {
    ...
    if (!request) return;
    setError(null);
    if (newPass.length < 8) return setError(t`Use at least 8 characters — a few plain words you'll remember.`);
    if (newPass !== confirm) return setError(t`The two passphrases don't match.`);
    setBusy(true);
  ...
  const guardianList = guardianCircle?.guardians.map((g) => g.email).join(", ") ?? "";

  return (
    <div className="gate">
      <div className="gate-card">
        <h1 className="gate-brand">Grove<span>.</span></h1>
        <h2><Trans>Ask your guardians</Trans></h2>

        {!account ? (
          <form onSubmit={signIn}>
            <p><Trans>Sign in with your account (not your passphrase) so your guardians can find your request.</Trans></p>
            {error ? <div className="error">{error}</div> : null}
            <label className="field">
              <span className="label"><Trans>Account email</Trans></span>
              <input type="email" autoFocus autoComplete="email" value={email} onChange={(e) => setEmail(e.target.value)} />
            </label>
            <label className="field">
              <span className="label"><Trans>Account password</Trans></span>
              <input type="password" autoComplete="current-password" value={password} onChange={(e) => setPassword(e.target.value)} />
            </label>
            <button type="submit" className="btn btn-primary" style={{ width: "100%" }} disabled={busy}>
              {busy ? t`Working…` : t`Sign in`}
            </button>
          </form>
        ) : !guardianCircle ? (
          <p>
          ...
          <>
            <p>
              {poll?.status === "pending_delay" && poll.readyAt
                ? t`Enough guardians have approved. Waiting out the safety window — ready in ${fmtCountdown(poll.readyAt - Date.now())}.`
                : t`Waiting for guardians to approve — ${approvals} of ${k} so far. Ask them to open the app.`}
            </p>
            <button className="btn btn-ghost" style={{ width: "100%" }} disabled={busy} onClick={cancel}>
              <Trans>Cancel this request</Trans>
            </button>
          </>
        ) : (
          <form onSubmit={finish}>
            <p><Trans>Your guardians have vouched for you. Choose a new passphrase.</Trans></p>
            {error ? <div className="error">{error}</div> : null}
            <label className="field">
              <span className="label"><Trans>New passphrase</Trans></span>
              <input type="password" autoFocus value={newPass} onChange={(e) => setNewPass(e.target.value)} />
            </label>
            <label className="field">
              <span className="label"><Trans>Type it again</Trans></span>
              <input type="password" value={confirm} onChange={(e) => setConfirm(e.target.value)} />
            </label>
            <button type="submit" className="btn btn-primary" style={{ width: "100%" }} disabled={busy}>
              {busy ? t`Finishing…` : t`Set new passphrase`}
            </button>
          </form>
        )}
